...
def run_two_stage_noPCA(X_scaled, n_micro=None, n_macro=None):
    ...
    print(f"\n[PHASE 2] Two-Stage Clustering WITHOUT PCA (Density-Based Strategy)")
    ...
    median_size = np.median(counts)
    MIN_MICRO_SIZE = max(3, int(median_size * 0.2)) # 20% της διαμέσου
...
def main():
    ...
    try:
        print(f"--- Processing: {filename} (DENSITY-BASED) ---")
    ...
    print("\n" + "="*50)
    print(f"FINAL SUMMARY (DENSITY-BASED)")
    print("="*50)
