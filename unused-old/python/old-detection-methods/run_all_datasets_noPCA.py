#!/usr/bin/env python3
"""
Runner Script για detection_noPCA.py - Τρέχει για όλα τα datasets
"""
...
def run_detection_noPCA_on_dataset(dataset_path):
    """Τρέχει το detection_noPCA.py για ένα dataset"""
    print("\n" + "="*80)
    print(f"🔍 Processing (NO PCA): {dataset_path}")
    print("="*80)
    ...
    try:
        result = subprocess.run(
            [sys.executable, "detection_noPCA.py", dataset_path],
...
def main():
    """Main function"""
    print("="*80)
    print("🚀 BATCH DETECTION (NO PCA) - Running on all datasets")
    print("="*80)
    ...
    for i, dataset in enumerate(datasets, 1):
        ...
        if run_detection_noPCA_on_dataset(dataset):
            successful += 1
        else:
            failed += 1
    ...
    print("\n\n" + "="*80)
    print("🎯 BATCH DETECTION (NO PCA) - FINAL SUMMARY")
    print("="*80)
