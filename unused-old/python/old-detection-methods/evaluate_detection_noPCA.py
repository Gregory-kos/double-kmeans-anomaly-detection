#!/usr/bin/env python3
"""
Evaluation Script για detection_noPCA.py
...
def load_detected_outliers_noPCA(dataset_name):
    """Φορτώνει τα detected outliers από το detection_noPCA.py output"""
    output_file = f"outputs/detection_noPCA/{dataset_name}/detection_noPCA_outliers_{dataset_name}.csv"
...
def evaluate_dataset(dataset_name, dataset_folder):
    ...
    print(f"\n{'='*80}")
    print(f"📊 Evaluating (NO PCA): {dataset_name}")
    print(f"{'='*80}")
    ...
    detected_df = load_detected_outliers_noPCA(dataset_name)
...
def main():
    """Main function"""
    print("="*80)
    print("🎯 EVALUATION (NO PCA) - Comparing Detected Outliers with Ground Truth")
    print("="*80)
    ...
    if all_metrics:
        ...
        output_file = "evaluation_results_noPCA.csv"
