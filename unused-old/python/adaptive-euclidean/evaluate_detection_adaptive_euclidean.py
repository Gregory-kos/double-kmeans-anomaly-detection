#!/usr/bin/env python3
"""
Evaluation script για Adaptive Euclidean Detection
...
def evaluate_dataset(dataset_name, dataset_path, output_base):
    ...
    print(f"\n{'='*80}")
    print(f"📊 Evaluating (ADAPTIVE EUCLIDEAN): {dataset_name}")
    print(f"{'='*80}")
    ...
    detected_file = os.path.join(output_base, dataset_name,
                                 f"adaptive_euclidean_outliers_{dataset_name}.csv")
    detected_outliers = load_detected_outliers(detected_file)

    if detected_outliers is None:
        print(f"   ⚠️ No detected outliers file found: {detected_file}")
        return None
...
def evaluate_basic_dataset(dataset_name, data_file, output_base):
    ...
    # Load ground truth from .env file
    env_file = "202526files/.env"
...
def main():
    print(f"{'='*80}")
    print(f"🎯 EVALUATION (ADAPTIVE EUCLIDEAN) - Comparing Detected Outliers with Ground Truth")
    print(f"{'='*80}")

    output_base = "outputs/detection_noPCA_adaptive_euclidean"
    results = []
    ...
    if results:
        ...
        print(f"\n{'='*80}")
        print(f"📋 SUMMARY TABLE (ADAPTIVE EUCLIDEAN DETECTION)")
        print(f"{'='*80}\n")
        print(df_results.to_string(index=False))

        csv_file = "evaluation_results_adaptive_euclidean.csv"
        df_results.to_csv(csv_file, index=False)
        print(f"\n✅ Results saved to: {csv_file}")

        # Calculate averages
        print(f"\n{'='*80}")
        print(f"📊 AVERAGE METRICS (ADAPTIVE EUCLIDEAN)")
        print(f"{'='*80}")
