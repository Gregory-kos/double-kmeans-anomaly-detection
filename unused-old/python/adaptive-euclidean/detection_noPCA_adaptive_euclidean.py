...
# EUCLIDEAN DISTANCE - απλή ευκλείδεια απόσταση
# =============================================================================

def calculate_euclidean_distances(points, center):
    """
    Υπολογίζει Euclidean distances από ένα σημείο (center) σε πολλά points
    ...
    Returns:
        distances: (N,) array - Euclidean distances
    """
...
def calculate_adaptive_safety_radius(X_used, final_labels, final_centers,
    ...
    print(f"\n[FULLY ADAPTIVE SAFETY RADIUS] Euclidean distance + relative cluster size")
    ...
    for cluster_id in np.unique(final_labels):
        ...
        # EUCLIDEAN DISTANCE
        distances = calculate_euclidean_distances(cluster_points, center)
...
def run_two_stage_noPCA_adaptive_euclidean(X_scaled, n_micro=None, n_macro=None):
    """
    Two-Stage Clustering με ADAPTIVE rules + EUCLIDEAN DISTANCE
    - Adaptive sigma (όχι fixed)
    - Adaptive safety radius percentile
    - Dynamic minimum micro-cluster size
    - Euclidean distance (όχι Mahalanobis)
    """
    ...
    print(f"\n[PHASE 2] Two-Stage Clustering WITHOUT PCA (Fully Adaptive Strategy - EUCLIDEAN)")
    print(f"   -> EUCLIDEAN DISTANCE - simple L2 norm")
    ...
    # 1. Υπολογισμός Micro-Distance με EUCLIDEAN
    print(f"   -> Computing Euclidean distances for micro-clusters...")
    ...
    for micro_id in range(n_micro):
        ...
        if mask.sum() > 0:
            ...
            micro_distances[mask] = calculate_euclidean_distances(cluster_points, center)

    # 2. Υπολογισμός MACRO-Distance με EUCLIDEAN
    print(f"   -> Computing Euclidean distances for macro-clusters...")
    ...
    for macro_id in range(n_macro):
        ...
        if mask.sum() > 0:
            ...
            macro_distances[mask] = calculate_euclidean_distances(cluster_points, center)
...
def plot_scaled_space(results_df, centers, filename):
    ...
    plt.title(f"Adaptive Two-Stage Clustering (NO PCA, EUCLIDEAN)\nFile: {filename}")
    ...
    output_dir = config.get_output_path('detection_noPCA_adaptive_euclidean', dataset_name, 'png')
    out_name = os.path.join(output_dir, f"plot_adaptive_euclidean_{dataset_name}.png")
...
def plot_raw_outliers(df_raw, df_clean, results_df, filename):
    ...
    plt.title(f"Adaptive Outlier Detection (EUCLIDEAN) - Original Coordinates\nFile: {filename}")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    dataset_name = filename.replace('.txt', '').replace('.csv', '')
    output_dir = config.get_output_path('detection_noPCA_adaptive_euclidean', dataset_name, 'png')
    out_name = os.path.join(output_dir, f"plot_adaptive_euclidean_raw_{dataset_name}.png")
    plt.savefig(out_name, dpi=150)
...
def main():
    parser = argparse.ArgumentParser(description='Adaptive Two-Stage Outlier Detection (Euclidean)')
    ...
    if filename is None:
        ...
        if not os.path.exists(filename):
            print("Usage: python detection_noPCA_adaptive_euclidean.py <filename>")
            sys.exit(1)
    ...
    try:
        print(f"{'='*80}")
        print(f"ADAPTIVE TWO-STAGE OUTLIER DETECTION (NO PCA, EUCLIDEAN)")
        print(f"{'='*80}")
    ...
    # Run adaptive algorithm with Euclidean distance
    results, centers = run_two_stage_noPCA_adaptive_euclidean(X_scaled)
    ...
    print("\n" + "="*80)
    print(f"FINAL SUMMARY (ADAPTIVE STRATEGY - EUCLIDEAN)")
    print("="*80)
    ...
    if num_outliers > 0:
        ...
        output_dir = config.get_output_path('detection_noPCA_adaptive_euclidean', dataset_name, 'csv')
        csv_name = os.path.join(output_dir, f"adaptive_euclidean_outliers_{dataset_name}.csv")
    ...
    print("\n" + "="*80)
    print("✅ Adaptive detection (Euclidean) completed successfully!")
    print("="*80)
