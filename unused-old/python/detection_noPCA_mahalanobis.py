...
# =============================================================================
def calculate_mahalanobis_distances(points, center, cov_matrix):
    """
    Υπολογίζει την απόσταση Mahalanobis για κάθε σημείο από το κέντρο

    Parameters:
    - points: array (n_samples, n_features)
    - center: array (n_features,)
    - cov_matrix: array (n_features, n_features)

    Returns:
    - distances: array (n_samples,)
    """
    try:
        # Υπολογισμός του αντίστροφου του covariance matrix
        # Χρησιμοποιούμε pinv για να αποφύγουμε προβλήματα με singular matrices
        cov_inv = pinv(cov_matrix)

        distances = np.array([
            np.sqrt(mahalanobis(point, center, cov_inv))
            for point in points
        ])
        return distances
    except Exception as e:
        print(f"   [Warning] Mahalanobis calculation failed, falling back to Euclidean: {e}")
        # Fallback to Euclidean if Mahalanobis fails
        return np.linalg.norm(points - center, axis=1)

def get_cluster_covariance(points, min_samples=3):
    """
    Υπολογίζει τον πίνακα συνδιακύμανσης για ένα cluster
    Προσθέτει regularization για stability
    """
    if len(points) < min_samples:
        # Αν έχουμε λίγα σημεία, επιστρέφουμε identity matrix
        return np.eye(points.shape[1])

    cov = np.cov(points.T)

    # Regularization: προσθέτουμε μικρή τιμή στη διαγώνιο για stability
    reg = 1e-6
...
# =============================================================================
# STEP 2: TWO-STAGE CLUSTERING WITHOUT PCA - MAHALANOBIS VERSION
# =============================================================================

def calculate_adaptive_safety_radius_mahalanobis(X_used, final_labels, final_centers,
                                                  percentile=90, min_radius=0.5, max_radius=5.0):
    """
    Υβριδική με MAHALANOBIS DISTANCE: Percentile με όρια (min/max) για ασφάλεια
    Επιστρέφει dictionary: {cluster_id: radius}
    """
    safety_radii = {}
    cluster_covs = {}

    print(f"\n[ADAPTIVE SAFETY RADIUS - MAHALANOBIS] Method: Hybrid (percentile={percentile})")
    ...
    for cluster_id in np.unique(final_labels):
        ...
        # Υπολογισμός covariance matrix για το cluster
        cov_matrix = get_cluster_covariance(cluster_points)
        cluster_covs[cluster_id] = cov_matrix

        # Υπολογισμός MAHALANOBIS distances
        distances = calculate_mahalanobis_distances(cluster_points, center, cov_matrix)
    ...
    return safety_radii, cluster_covs
...
def run_two_stage_noPCA_mahalanobis(X_scaled, n_micro=None, n_macro=None):
    ...
    print(f"\n[PHASE 2] Two-Stage Clustering WITHOUT PCA - MAHALANOBIS DISTANCE")
    ...
    # --- E. CALCULATE COVARIANCE MATRICES FOR MICRO AND MACRO CLUSTERS ---
    print("\n[CALCULATING COVARIANCE MATRICES]")

    # Micro cluster covariances
    micro_covs = {}
    for i in range(n_micro):
        mask = (micro_labels == i)
        points = X_used[mask]
        micro_covs[i] = get_cluster_covariance(points)

    # Macro cluster covariances (calculated in safety radius function)

    # --- F. HYBRID OUTLIER DETECTION WITH MAHALANOBIS ---

    # 1. Υπολογισμός Micro-Distance (MAHALANOBIS)
    micro_distances = np.zeros(len(X_used))
    for i in range(len(X_used)):
        cluster_id = micro_labels[i]
        center = micro_centers[cluster_id]
        cov = micro_covs[cluster_id]
        try:
            cov_inv = pinv(cov)
            micro_distances[i] = np.sqrt(mahalanobis(X_used[i], center, cov_inv))
        except:
            # Fallback to Euclidean
            micro_distances[i] = np.linalg.norm(X_used[i] - center)

    # 2. Υπολογισμός MACRO-Distance (MAHALANOBIS) και Safety Radii
    safety_radii_dict, macro_covs = calculate_adaptive_safety_radius_mahalanobis(
    ...
    for i in range(len(X_used)):
        ...
        center = final_centers[cluster_id]
        cov = macro_covs[cluster_id]
        try:
            cov_inv = pinv(cov)
            macro_distances[i] = np.sqrt(mahalanobis(X_used[i], center, cov_inv))
        except:
            # Fallback to Euclidean
            macro_distances[i] = np.linalg.norm(X_used[i] - center)
    ...
    # 4. Προσθήκη του safety_radius στο DataFrame
    ...
    # 6. ΤΕΛΙΚΗ ΑΠΟΦΑΣΗ ΜΕ ΤΗΝ "ΑΣΠΙΔΑ"
    ...
    # Print Stats
    print("\n[CLUSTER STATISTICS - MAHALANOBIS]")
...
def plot_scaled_space(results_df, centers, filename):
    """Plot in scaled space (algorithm workspace) - NO PCA - MAHALANOBIS"""
    ...
    plt.title(f"Two-Stage Clustering - MAHALANOBIS DISTANCE (Scaled Space)\nFile: {filename}")
    ...
    output_dir = config.get_output_path('detection_noPCA_mahalanobis', dataset_name, 'png')
    out_name = os.path.join(output_dir, f"plot_mahalanobis_detection_{dataset_name}.png")
...
def plot_raw_outliers(df_raw, df_clean, results_df, filename):
    ...
    plt.title(f"Outliers in Original Coordinates - MAHALANOBIS\nFile: {filename}")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    dataset_name = filename.replace('.txt', '').replace('.csv', '')
    output_dir = config.get_output_path('detection_noPCA_mahalanobis', dataset_name, 'png')
    out_name = os.path.join(output_dir, f"plot_mahalanobis_raw_outliers_{dataset_name}.png")
    plt.savefig(out_name, dpi=150)
...
def main():
    ...
    try:
        print(f"--- Processing: {filename} (MAHALANOBIS DISTANCE) ---")
    ...
    # Algorithm WITHOUT PCA with MAHALANOBIS distance
    results, centers = run_two_stage_noPCA_mahalanobis(X_scaled)
    ...
    print("\n" + "="*50)
    print(f"FINAL SUMMARY (MAHALANOBIS DISTANCE)")
    print("="*50)
    ...
    if num_outliers >= 0:
        ...
        output_dir = config.get_output_path('detection_noPCA_mahalanobis', dataset_name, 'csv')
        csv_name = os.path.join(output_dir, f"detection_mahalanobis_outliers_{dataset_name}.csv")
