#!/usr/bin/env python3
"""
Evaluation Script για detection_stability.py
...
def load_detected_outliers_stability(dataset_name):
    """Φορτώνει τα detected outliers από το detection_stability.py output"""
    output_file = f"outputs/detection_stability/{dataset_name}/stability_outliers_{dataset_name}.csv"

    if not os.path.exists(output_file):
        print(f"   ⚠️  Warning: Output file not found: {output_file}")
        return None

    df_detected = pd.read_csv(output_file, header=None, names=['x', 'y', 'instability'])
    return df_detected
...
def evaluate_dataset(dataset_name, dataset_folder):
    ...
    print(f"\n{'='*80}")
    print(f"📊 Evaluating (STABILITY): {dataset_name}")
    print(f"{'='*80}")
    ...
    detected_df = load_detected_outliers_stability(dataset_name)
...
def main():
    """Main function"""
    print("="*80)
    print("🎯 EVALUATION (STABILITY) - Comparing Detected Outliers with Ground Truth")
    print("="*80)
    ...
    if all_metrics:
        print("\n\n" + "="*80)
        print("📋 SUMMARY TABLE (STABILITY-BASED DETECTION)")
        print("="*80)
        ...
        output_file = "evaluation_results_stability.csv"
        df_summary.to_csv(output_file, index=False)
        print(f"\n✅ Results saved to: {output_file}")

        print("\n" + "="*80)
        print("📊 AVERAGE METRICS (STABILITY-BASED)")
        print("="*80)
