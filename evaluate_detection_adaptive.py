#!/usr/bin/env python3
"""
Evaluation Script for detection_noPCA_adaptive.py
Calculates: TP, TN, FP, FN, Precision, Recall, F1-Score
"""
...
def load_detected_outliers_adaptive(dataset_name):
    """Load detected outliers from detection_noPCA_adaptive.py output"""
    output_file = f"outputs/{dataset_name}/adaptive_outliers_{dataset_name}.csv"
...
def match_outliers_by_coordinates(ground_truth_df, detected_df, tolerance=1e-6):
    """Match outliers based on coordinates"""
...
def evaluate_dataset(dataset_name, dataset_folder):
    """Evaluate a dataset"""
    print(f"\n{'='*80}")
    print(f"📊 Evaluating (ADAPTIVE): {dataset_name}")
    print(f"{'='*80}")
    ...
    detected_df = load_detected_outliers_adaptive(dataset_name)
...
def main():
    """Main function"""
    print("="*80)
    print("🎯 EVALUATION (ADAPTIVE) - Comparing Detected Outliers with Ground Truth")
    print("="*80)
    ...
    if all_metrics:
        print("\n\n" + "="*80)
        print("📋 SUMMARY TABLE (ADAPTIVE DETECTION)")
        print("="*80)
        ...
        output_file = "evaluation_results_adaptive.csv"
        df_summary.to_csv(output_file, index=False)
        print(f"\n✅ Results saved to: {output_file}")

        print("\n" + "="*80)
        print("📊 AVERAGE METRICS (ADAPTIVE)")
        print("="*80)
